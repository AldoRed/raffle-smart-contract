...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          let raffle, vrfCoordinatorV2Mock, chai, assert, expect, raffleEntranceFee, deployer, interval
          ...
          describe("constructor", function () {
              it("initializes constructor correctly", async function () {
                  ...
                  assert.equal(entraceFee.toString(), networkConfig[chainId].entranceFee)
              })
          })
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("can only be called after performUpKeep", async function () {
                  await expect(vrfCoordinatorV2Mock.fulfillRandomWords(0, raffle.target)).to.be.rejectedWith(
                      "nonexistent request"
                  )
                  await expect(vrfCoordinatorV2Mock.fulfillRandomWords(1, raffle.target)).to.be.rejectedWith(
                      "nonexistent request"
                  )
              })
              it("picks a winner, resets the lottery, and sends money", async function () {
                  ...
                  await new Promise(async (resolve, reject) => {
                          ...
                          try {
                              ...
                              assert.equal(
                                  ...
                                  Number(winnerStartingBalance) +
                                      Number(raffleEntranceFee) * Number(aditionalsEntrants) +
                      ...
                      await vrfCoordinatorV2Mock.fulfillRandomWords(txReceipt.logs[1].args[0], raffle.target)
                  })
              })
          })
      })
